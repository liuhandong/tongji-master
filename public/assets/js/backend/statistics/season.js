    ...
    var Controller = {
		...
		//重置
			...
			else
			{
				...
				window.location.href="season/print_excel?mondata="+$("#mondata").val()+'&companydata='+$("#companydata").val()+'&objdata='+$("#objdata").val()+'&searchType='+searchType;
			}	
		});
		 // 为表格绑定事件
          $("#submit").click(function(){
			...
			else
			{
		             ...
		             $.ajax({url:"statistics/season/getObjData",success:function(result){
        ...
        edit: function () {
        	   $("#export_excel").click(function(){
				...
				url3=url.length-url2.length;
        	  	window.location.href="http://"+window.location.hostname+url.substring(0,url3)+"declared/declareds/print_excel?ids="+$("#ids").val();
        	  });
