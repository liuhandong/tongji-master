...
class Season extends Backend
{
    ...
    public function index(){
    	   ...
    	   {
    	   		$mondata[$i.'-'."01"] =$i.'年第一季度';
    	   		$mondata[$i.'-'."02"] =$i.'年第二季度';
    	   		$mondata[$i.'-'."03"] =$i.'年第三季度';
    	   		$mondata[$i.'-'."04"] =$i.'年第四季度';
    	   }
        //快捷查询数组数组联合查询需要带表名
        ...
        if ($this->request->isAjax()){
		  ...
		  $mondata_re = $this->request->get("mondata");
            ...
            //echo $this->sql->getDeclaredsList($where,$sort,$order,$offset,$limit);
			...
			if($searchType==1)
			{
				...
				{
					$row = Db::query($this->sql->getDeclaredsRow($val['id']));
    ...
    public function edit($ids = NULL){
	  ...
	  //$file = $url_arr[0]."public/".$rows['file_path'];
	   ...
	   if ($this->request->isPost()){
			...
			$list=Db::getOne($this->sql->getDeclaredsRowToMsg($ids));
			$adata['sender_id'] = $admin['id'];
			$adata['content'] = $list['company_park_name'].$list['mon']."季报已经被驳回，请确认!";
			$adata['add_time'] = time();
