...
class Year extends Backend
{
    ...
    public function edit($ids = NULL){
	  ...
	  $url_arr=explode("public",$url);
	   ...
	   if ($this->request->isPost()){
			...
			$list=Db::getOne($this->sql->getDeclaredyRowToMsg($ids));
			$adata['sender_id'] = $admin['id'];
			$adata['content'] = $list['company_park_name'].$list['mon']."年报已经被驳回，请确认!";
			$adata['add_time'] = time();
    ...
    public function lastMonth($month){
        $month = date("Y",strtotime("-1 year",strtotime($month)));
        return $month;
    }
