 ...
 * Time: 15:33
 */
class Declaredsyr extends Backend
{
    protected $sql= null;
    ...
    public function uploadlist($ids = NULL){
        if ($this->request->isAjax()){//isAjax
		  ...
		  //$file = $url_arr[0]."public/".$rows['file_path'];
            ...
            $title=$row['company_park_name'].' '.$row['mon'].' 年报表 查看附件';
    ...
    public function add(){
        ...
        if ($this->request->isPost()){
            ...
            {
	            $isin = Db::table('zc_year')->insert($adata);
	            $max_id = Db::table('zc_year')->max('id');
	            $params['fa_id'] = $max_id;
    ...
    public function tibao($ids=Null){
		...
		if($admin['company_id']==$list['declared_company_id'])
		{
	        ...
	        if($bool){
	            ...
	            $adata['content'] = $list['company_park_name'].$list['mon']."年报已经提交，请进行校对!";
	            $adata['add_time'] = time();
