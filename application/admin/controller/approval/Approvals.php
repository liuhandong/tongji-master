...
use think\Db;
use app\admin\sql\approvalsSql;
use think\Session;
use think\Log;

class Approvals extends Backend
{
    protected $sql= null;
    public function _initialize()
    {
        parent::_initialize();
        $this->sql = new approvalsSql();
    }
    ...
    public function yessp($ids=Null){
        ...
        if($bool){
			...
			$adata['content'] = $list['company_park_name'].$list['mon']."季报已经审批。";
			$adata['add_time'] = time();
    ...
    public function nosp($ids=Null){
        ...
        if ($this->request->isPost()){
			...
			{
				$adata['sender_id'] = $admin['id'];
				$adata['content'] = "您提交的".$list['company_park_name'].$list['mon']."季报已经被驳回，请确认!";
				$adata['add_time'] = time();
				$adata['recipient_id'] = $val['uid'];
				$adata['read_flg'] ="0";
				$isin = Db::table('zc_message')->insert($adata);
