...
LEFT JOIN zc_company zc ON zy.declared_company_id = zc.id WHERE zy.id = %d', $id);
    ...
    public function getLastYearDeclaredsList($mon,$company_id){
        $sql = sprintf("SELECT * FROM `zc_year` as zs left join zc_swj_report_year as zsr on zs.id=zsr.fa_id WHERE mon='%s' and declared_company_id='%d'", $mon,$company_id);
        return $sql;
    }
    ...
    public function insertDeclaredy($data)
    {
        ...
        foreach ($data as $key => $value) {
            $sql = sprintf('INSERT INTO zc_swj_report_year(rf_id,item_val,add_time,fa_id)VALUES(%d,\'%s\',%d,%d)', $key, $value, $add_time,$fa_id);
...
LEFT JOIN zc_swj_unit zsu ON zsu.id = zsrf.unit_id WHERE  zsrf.pid is null and zsrm.fa_id = %d', $id);
    ...
    public function updateDeclaredy($data){
        $sql = sprintf('UPDATE zc_swj_report_year SET item_val=\'%s\' WHERE id=%d',$data['item_val'], $data['id']);
        return $sql;
    }
